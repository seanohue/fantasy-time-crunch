...
const { createIdList, handleConfigErrors, handleErrors, maybeCallWithContext } = require("../lib/utils");

const testCases = [null, undefined, 1, "2", {}, true, false, [], new Set(), new Map(), Symbol];
  ...
  describe("[Function createIdList]", () => {
    describe("functionality:", () => {
      ...
      it("should return an empty string if there are invalid args", () => {
        const testCases = [, "test", {}, false, true, function () {}, 1, new Set([{ id: "4" }]), new Map()];
        expect(createIdList(testCases)).to.eq("");
      });

      it("should filter non-number ids", () => {
        const output = createIdList([{ id: {} }, { id: false }, { id: true }, { id() {} }, { id: "2" }, { id: 1 }]);
  ...
  describe("[Function handleErrors]", () => {
    describe("functionality:", () => {
      ...
      it("should throw an error message when errors array has length", () => {
        ...
        const err = "test";
        expect(() => handleErrors("first", "second", { errors: [err] })).to.throw(
    ...
    describe("errors:", () => {
      ...
      it("should throw an error when the first arg is not a string", () => {
        testCases
          ...
          .forEach((testCase) => {
            expect(() => handleErrors(testCase, "test", { errors: [] })).to.throw(
      ...
      it("should throw an error when the second arg is not a string", () => {
        testCases
          .filter((testCase) => typeof testCase !== "string")
          .forEach((testCase) => {
            expect(() => handleErrors("test", testCase, { errors: [] })).to.throw(
  ...
  describe("[Function handleConfigErrors]", () => {
    describe("functionality:", () => {
      ...
      it("should throw an error message when errors array has length", () => {
        ...
        expect(() => handleConfigErrors({ errors: [err] })).to.throw("Invalid TimeSystem Configuration, see logs");
        expect(stub).to.have.calledWith("Configuration Error: ", err);
  ...
  describe("[Function maybeCallWithContext]", () => {
    ...
    describe("errors:", () => {
      it("should throw an error when no arg is passed", () => {
        expect(() => maybeCallWithContext()).to.throw("ctx is not a Constructor");
      });
